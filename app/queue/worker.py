...
class BackgroundWorker:
    ...
    def _process_job(self, job: dict):
        ...
        try:
            ...
            no_verse_match = context.get('no_verse_match', False)
            closest_ayah_suggestions = context.get('closest_ayah_suggestions', [])
            expected_ayah_candidate = context.get('expected_ayah_candidate')
            expected_ayah_analysis = context.get('expected_ayah_analysis')
            ...
            if verse_details:
                ...
                self.logger.warning(
                    f"[{job_id}] No verse match found. Completing job with metadata-only response."
                )
            else:
            ...
            tutor_statuses = [v.get('tutor_status') for v in verse_details if v.get('tutor_status')]
            matching_warnings = getattr(context, 'matching_warnings', [])
            tutor_summary = {
                'total_assessed_ayahs': len(tutor_statuses),
                'correct_ayahs': sum(1 for s in tutor_statuses if s == 'correct'),
                'mistake_ayahs': sum(1 for s in tutor_statuses if s == 'mistake'),
                'needs_review_ayahs': sum(1 for s in tutor_statuses if s == 'needs_review'),
                'matching_warning_count': len(matching_warnings)
            }
            ...
            metadata = {
                ...
                "ayahs": verse_details,
                "closest_ayah_suggestions": closest_ayah_suggestions,
                "expected_ayah_candidate": expected_ayah_candidate,
                "expected_ayah_analysis": expected_ayah_analysis,
                ...
                "diagnostics": {
                    ...
                    "no_verse_match": no_verse_match,
                    "has_closest_ayah_suggestions": len(closest_ayah_suggestions) > 0,
            ...
            job_queue.update_job_status(
                job_id,
                JobStatus.COMPLETED,
                result_zip_path=str(result_zip_path) if result_zip_path else None,
