...
class TimestampCalculationStep(PipelineStep):
    ...
    def process(self, context: PipelineContext) -> PipelineContext:
        ...
        if context.get('no_verse_match', False) and not context.matched_chunk_verses:
            context.verse_slices_timestamps = []
        ...
        for chunk_data in updated_matched_chunk_verses:
            ...
            for ayah in matched_ayahs:
                ...
                if verse_key not in verse_to_chunks:
                    ...
                    if ayah.get('tutor_assessment'):
                        verse_to_chunks[verse_key]['tutor_assessment'] = ayah.get('tutor_assessment')
                ...
                if chunk_data.get('tutor_status'):
                    chunk_info['tutor_status'] = chunk_data.get('tutor_status')
                if chunk_data.get('tutor_mistakes') is not None:
                    chunk_info['tutor_mistakes'] = chunk_data.get('tutor_mistakes')
        ...
        for verse_key in sorted(verse_to_chunks.keys()):
            ...
            if verse_data.get('tutor_assessment'):
                verse_entry['tutor_assessment'] = verse_data['tutor_assessment']
                verse_entry['tutor_status'] = verse_data['tutor_assessment'].get('status')
